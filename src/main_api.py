...
app = FastAPI(
    ...
    1. **Generate Keywords**: POST `/keywords/generate` with your research idea
    2. **Search PubMed**: POST `/search/pubmed` with the generated keywords
    3. **View Results**: GET `/articles` to see all stored articles
    4. **Check Scores**: GET `/search/{search_id}/scores` for opportunity analysis
    ...
    curl -X POST "http://localhost:8000/keywords/generate" \\
         ...
         -d '{"idea": "machine learning in cancer diagnosis"}'
    ...
    curl -X POST "http://localhost:8000/search/pubmed" \\
         ...
         -d '{
           ...
           "idea_text": "machine learning in cancer diagnosis",
           "max_results": 20
         }'
    ...
    ### Analysis Endpoints
    - `GET /search/{search_id}/scores` - Get opportunity scores
    ...
    - **PubMedBERT**: For keyword extraction and semantic similarity
    - **KeyBERT**: For keyword generation with MMR diversity
    - **HDBSCAN**: For article clustering
    - **ARIMA**: For citation forecasting
    
    ## 📈 Opportunity Scoring
    
    The API computes three key metrics:
    
    - **Novelty Score**: How unique the research topic is
    - **Citation Velocity**: Rate of citation growth
    - **Recency Score**: How recent the research is
    
    These are combined into an overall opportunity score (0.0 to 1.0).
    ...
    version="1.0.0",
    contact={
        "name": "Prime Time API Support",
        "email": "support@primetime-research.com",
    },
    license_info={
        "name": "MIT License",
